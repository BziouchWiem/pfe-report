...
\begin{itemize}
  ...
  \item \textbf{Extensibilité des moteurs} : capacité à brancher de nouveaux
  scanners et à normaliser leurs sorties vers le même modèle commun, sans modifier
  le cœur métier.
...
et des dépôts. L'epic \textit{Scans SAST/DAST} porte sur l'orchestration
...
5 & Gestion des projets & 5.1
  & En tant que développeur, je veux créer un projet et définir ses URLs cibles
    afin de centraliser la configuration des analyses. & M \\
5 & Gestion des projets & 5.2
  & En tant que développeur, je veux configurer les paramètres de scan par projet
    afin d'adapter les analyses au contexte applicatif. & S \\
6 & Gestion des membres & 6.1
  & En tant que développeur, je veux inviter des membres et leur attribuer des rôles
...
\hline
\rowcolor{gray!10}
\multicolumn{5}{c}{\textbf{Module Scans SAST / DAST et Orchestration}} \\
\hline
...
12 & Orchestration asynchrone & 12.1
...
\begin{table}[H]
\centering
\caption{Product Backlog -- Workflow, IA et reporting}
...
18 & Workflow vulnérabilités & 18.1
  & En tant que Security Manager, je veux changer l'état d'une vulnérabilité et l'assigner
    à un responsable afin de structurer le processus de remédiation. & M \\
18 & Workflow vulnérabilités & 18.2
  & En tant que développeur, je veux changer l'état d'une vulnérabilité. & M \\
...
\begin{table}[H]
\centering
\caption{Product Backlog -- CI/CD et notifications}
\label{tab:backlog-cicd-notif}
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\textwidth}{c X c X c}
\hline
...
\textbf{quatre mois}, découpée en six sprints de deux à trois semaines chacun,
...
une logique de construction en couches : fondations techniques (Sprint~0), gouvernance des
accès (Sprint~1), gestion des projets (Sprint~2), analyses de sécurité SAST et DAST (Sprint~3),
workflow collaboratif de remédiation (Sprint~4), puis restitution analytique et assistance IA (Sprint~5).
...
2 & \textbf{Projets et intégrations}
  ...
  & 3~semaines \\
\hline
3 & \textbf{SAST et DAST}
  & Intégration Semgrep et Bandit, CLI~scanner, normalisation des alertes SAST~;
    intégration OWASP~ZAP en mode daemon, workers Celery/Redis, suivi temps réel
    Socket.IO, scoring de risque.
  & Pipeline SAST opérationnel, pipeline DAST asynchrone, score de risque calculé.
  & 3~semaines \\
\hline
4 & \textbf{Workflow de vulnérabilités}
  & Synchronisation des tâches de remédiation, assignation aux développeurs,
    machine d'états (\texttt{open\,→\,in\_progress\,→\,fixed\,→\,verified}),
    commentaires collaboratifs, notifications temps réel.
  & Système de gestion des tâches opérationnel, notifications Socket.IO fonctionnelles.
  & 3~semaines \\
\hline
5 & \textbf{Dashboard, Recommandation LLM et Rapport}
  & Tableau de bord agrégé (métriques, tendances)~; assistant IA local
    (\textit{Assist with AI}) via Ollama (Mistral/Qwen)~;
    génération de rapports de sécurité PDF/JSON (ReportLab).
  & Dashboard opérationnel, recommandations IA persistées, rapports exportables.
  & 3~semaines \\
\hline
\end{tabularx}
\end{table}
...
\placeholderlogo
  ...
  & Exécute les analyses SAST et DAST dans des workers isolés sans bloquer
...
\begin{itemize}
  ...
  (SAST, DAST) via des workers Celery, avec file d'attente Redis et suivi
  de progression.
...
en six sprints garantissent une progression maîtrisée, alignée sur la valeur métier
